...
public class ItemDragHelperCallback extends ItemTouchHelper.Callback {
    ...
    public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, RecyclerView.ViewHolder target) {
        // 固定栏目不能移动
        if(target instanceof SubscribeAdapter.MyViewHolder) {
            if(((SubscribeAdapter.MyViewHolder) target).isFix) {
                return false;
            }
        }
        if(viewHolder instanceof SubscribeAdapter.MyViewHolder) {
            if(((SubscribeAdapter.MyViewHolder) viewHolder).isFix) {
                return false;
            }
        }
